...
int main(int argc, char** argv) {
    ...
    // Usage:
    // ./test_histogram [input_dir] [output_dir] [impl]
    // Example:
    // ./test_histogram data/png data/tests/hist_results simd
    try {
        if (argc >= 2) input_dir  = argv[1];
        if (argc >= 3) output_dir = argv[2];
        if (argc >= 4) impl       = parse_impl(argv[3]);

        return alpha_hist::run_histogram_tests(input_dir, output_dir, impl);
